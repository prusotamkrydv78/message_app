...
export const api = {
  ...
  deleteConversation: (token, id) => request(`/api/v1/conversations/${encodeURIComponent(id)}`, { method: "DELETE", token }),
  createCall: (token, data) => request("/api/v1/calls", { method: "POST", token, body: data }),
  createTestCall: (token, data) => request("/api/v1/calls/test", { method: "POST", token, body: data }),
  callHistoryWith: (token, userId) => request(`/api/v1/calls/with/${encodeURIComponent(userId)}`, { token }),
};
